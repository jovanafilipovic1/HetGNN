...
ppi_interactions = ppi_obj.getInteractionNamed() #function defined in multigraph.py, it returns a dataframe with 3 columns "Gene_A", "Gene_B", interaction type
...
if cell_feat_name == "expression":
    ...
    #add the mean variance as the expression for the missing features
    ...
    cancer_expression_full[:] = np.random.uniform(0, 10, size=cancer_expression_full.shape) #delete this line!!!
    ...
    hvg_final = ccle_cnv.std()[ccle_cnv.std() >= hvg_q].index #select genes with high variance
